...
public class UtilsMixin {
	...
	private static boolean useNumismaticCost() {
		return getTeleportCostItem() != null && (getTeleportCostItem().equals(copperId) || getTeleportCostItem().equals(silverIO) || getTeleportCostItem().equals(goldId));
	}

	private static int getNumismaticFactor() {
		return getTeleportCostItem().equals(copperId) ? 1 : getTeleportCostItem().equals(silverIO) ? 100 : 10000;
	}
	...
	public static boolean containsItem(PlayerInventory inventory, Item item, int maxAmount) {
		...
		if (useNumismaticCost()) {
			return maxAmount*getNumismaticFactor() <= (int) ModComponents.CURRENCY.get(inventory.player).getValue();
		} else {
			int amount = 0;
	...
	public static void removeItem(PlayerInventory inventory, Item item, int totalAmount) {
		...
		Identifier itemId = getTeleportCostItem();
		if (useNumismaticCost()) {
			ModComponents.CURRENCY.get(inventory.player).modify(-getNumismaticFactor()*totalAmount);
		} else {
